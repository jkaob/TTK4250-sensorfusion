...
# literally no difference in the plots regardless of sigma
...
x_bar_init = np.array([0,100,1,1])
P_bar_init = np.diag([50,50,5,5])  # TODO: ArrayLike with 4 x 4 elements, hint: np.diag
...
RMSE_pred = np.array([0,0])
#RMSE_pred= np.sqrt(np.mean(np.square(stats['dists_pred']), axis=1))
RMSE_pred[0] = np.sqrt( pow(stats['dists_pred'][:,0],2).mean() )
RMSE_pred[1] = np.sqrt( pow(stats['dists_pred'][:,1],2).mean() )

RMSE_upd = np.array([0,0])
#RMSE_upd = np.sqrt(np.mean(np.square(stats['dists_upd']), axis=1))  
RMSE_upd[0] = np.sqrt( pow(stats['dists_upd'][:,0],2).mean() )
RMSE_upd[1] = np.sqrt( pow(stats['dists_upd'][:,1],2).mean() )
...
RMSE_upd = np.sqrt(np.mean(np.square(stats_array['dists_pred']), axis=2))
...
ax4.plot_surface(*np.meshgrid(sigma_a_list, sigma_z_list), ANIS, alpha=0.9)
ax4.contour(*np.meshgrid(sigma_a_list, sigma_z_list), ANIS, [1, 1.5, *CINIS, 2.5, 3], offset=0)  # , extend3d=True, colors='yellow')
