...
class EKFSLAM:
    ...
    def predict(
        ...
        #P[:3, :3] = Fx @ P[:3,:3] @ Fx.T + self.Q # robot cov prediction
        P[:3, :3] = Fx @ P[:3,:3] @ Fx.T + Fu @ self.Q @ Fu.T # G = Fu ?
    ...
    def h(self, eta: np.ndarray) -> np.ndarray:
        ...
        zpredcart   = delta_m - R @ self.sensor_offset

        zpred_r     = la.norm(zpredcart) # ranges
        zpred_theta = np.arctan(zpredcard[1], zpredcart[0]) # bearings
        zpred       = np.vstack(zpred_r, zpred_theta)
        # [ranges; bearings] shape (2, #lmrk)
    ...
    def H(self, eta: np.ndarray) -> np.ndarray:
        ...
        delta_m = m - x[:2] # relative position of landmark to robot in world frame. m - rho that appears in (11.15) and (11.16)
        zc      = delta_m - R @ self.sensor_offset
