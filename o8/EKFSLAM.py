...
class EKFSLAM:
    ...
    def h(self, eta: np.ndarray) -> np.ndarray:
        ...
        delta_m     = m - x[:2]  # No sensor offset here
        ...
        zpred_theta = np.arctan(zpredcard[1], zpredcart[0])
    ...
    def H(self, eta: np.ndarray) -> np.ndarray:
        ...
        for i in range(numM):  # But this whole loop can be vectorized
            ...
            Hx[inds]        = np.vstack((zc.T / la.norm(zc)) @ temp,
                                        (zc.T @ Rpihalf.T / la.norm(zc)**2) @ temp)
    ...
    def add_landmarks(
        ...
        for j in range(numLmk):
            ...
            rot          = rotmat2d(zj[1] + eta[2])  # R(zj_angle + psi) #rotmat in Gz
            lmnew[inds]  = rotmat2d(eta[2]) @ lmnew[inds] # calculate position of new landmark in world frame
